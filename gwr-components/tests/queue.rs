...
fn queue_rejects_zero_capacity() {
    ...
    let err = Queue::<usize>::new(engine.top(), "queue", Some(0)).unwrap_err();
...
fn queue_push_pop_fifo_and_reports_state() {
    let engine = start_test(file!());
    let queue = Queue::new(engine.top(), "queue", Some(2)).unwrap();
...
fn queue_with_no_capacity_limit_accepts_many_values() {
    ...
    let engine = start_test(file!());
    let queue = Queue::new(engine.top(), "queue", None).unwrap();
...
fn queue_remove_where_removes_first_match() {
    let engine = start_test(file!());
    let queue = Queue::new(engine.top(), "queue", Some(4)).unwrap();
...
fn bounded_queue_push_waits_until_space_is_available() {
    ...
    let queue = Rc::new(Queue::new(engine.top(), "queue", Some(1)).unwrap());
...
fn queue_changed_event_fires_for_mutations() {
    let mut engine = start_test(file!());
    let clock = engine.default_clock();
    let queue = Rc::new(Queue::new(engine.top(), "queue", Some(2)).unwrap());
