...
fn build_stdout_tracker(level: log::Level, filter_regex: &str) -> Tracker {
    ...
    Arc::new(TextTracker::new(entity_manager, stdout_writer))
}

/// Same as the text tracker (see build_stdout_tracker) except will generate a
/// binary file.
fn build_binary_tracker(level: log::Level, filter_regex: &str, trace_file: &str) -> Tracker {
    let default_level = if filter_regex.is_empty() {
        level
    } else {
        log::Level::Error
    ...
    Arc::new(CapnProtoTracker::new(entity_manager, bin_writer))
}
