...
//! # Steam Command-Line Assistant
//!
//! `Terminus` is a command-line utility to aid in the learning / development of
//! Steam.
...
use terminus::CliLogger;
use terminus::recipe::{Recipe, run_command_as_interactive};
use terminus::tui::Tui;

/// Command-line arguments.
#[derive(Parser)]
#[command(about = "Recipe creator")]
struct Cli {
...
fn write_recipe(
    ...
    // command-line
    let mut app = terminus::writer::app::App::new(history_path, "", false);
...
fn start_write_recipe_tui(history: &PathBuf, recipes_folder: &str) -> Result<()> {
    color_eyre::install()?;
    let mut app = terminus::writer::app::App::new(history, recipes_folder, true);
    ...
    while app.is_running() {
        ...
        if let Event::Key(key_event) = event::read()? {
            terminus::writer::handler::handle_key_event(key_event, &mut app);
        }
    }
...
fn start_run_recipe_tui(recipes_folder: &str, tmp_root: &Path, keep_tmp: bool) -> Result<()> {
    color_eyre::install()?;
    let mut app = terminus::runner::app::App::new(recipes_folder, tmp_root, keep_tmp);
    ...
    while app.is_running() {
        ...
        // Handle events.
        if let Event::Key(key_event) = event::read()? {
            terminus::runner::handler::handle_key_event(key_event, &mut app);
        }
    }
