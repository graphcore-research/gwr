...
(() => {
  ...
  function appendFilterOptions(container, values, selected) {
  ...
  function ensureFilterOptions(picker) {
    ...
    appendFilterOptions(picker.container, picker.values, picker.selected);
  ...
  function layerOverviewTraffic(layer, unfiltered, filteredContext) {
    if (unfiltered) {
      return {
        readBytes: toBigInt(layer.tensor_read_bytes),
        writeBytes: toBigInt(layer.tensor_write_bytes),
        tensorCount: Number(layer.tensor_count || 0),
      };
    }
    const context = filteredContext();
    return {
      readBytes: context.readBytes,
      writeBytes: context.writeBytes,
      tensorCount: context.tensors.length,
    };
  }
  ...
  function connectionTrafficFor(
    ...
    if (isAllFilter(layerSelection)) {
      return {
        bytes: toBigInt(connection.bytes),
        edgeCount: Number(connection.edge_count || 0),
      };
    }
    return [...filterValues(layerSelection)].reduce(
      (total, layerName) => {
        const traffic = connection.by_layer?.[layerName];
        total.bytes += toBigInt(traffic?.bytes);
        total.edgeCount += Number(traffic?.edge_count || 0);
        return total;
      },
      { bytes: 0n, edgeCount: 0 },
    );
  }
  ...
  function visibleConnections(
    ...
    return (connections || [])
      ...
      .map((connection) => ({
        ...connection,
        ...connectionTrafficFor(connection, layerSelection),
      }))
      .filter(
  ...
  function tensorMemoryOverlapBytes(
    ...
    const overlap = (data.memory?.platform_memories || [])
    ...
    return overlap < tensorBytes ? overlap : tensorBytes;
  }
  ...
  function scaleTensorToMemory(
    ...
    return scaleInteger(
      bytes,
      tensorMemoryOverlapBytes(tensor, memorySelection),
      tensorBytes,
    );
  }
  ...
  function contextSnapshot(
    ...
    const snapshot = (data.tensors || []).reduce(
      (result, tensor) => {
        ...
        const traffic = tensorTrafficFor(tensor, layerSelection, peSelection);
        if (
          trafficUnfiltered ||
          traffic.edgeCount > 0 ||
          traffic.readBytes > 0 ||
          ...
          result.readBytes += scaleTensorToMemory(
            tensor,
            traffic.readBytes,
            memorySelection,
          );
          result.writeBytes += scaleTensorToMemory(
            tensor,
            traffic.writtenBytes,
            memorySelection,
          );
          result.edges += traffic.edgeCount;
        }
        return result;
      },
      { tensors: [], readBytes: 0n, writeBytes: 0n, edges: 0 },
    );
  ...
  function filteredSummary() {
    ...
    return {
      ...
      writeBytes: unfiltered
        ...
        : context.writeBytes,
      edges: unfiltered ? Number(data.summary.edges || 0) : context.edges,
    };
  }
