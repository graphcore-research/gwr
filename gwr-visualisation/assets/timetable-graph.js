...
(() => {
  ...
  let graphDataInitialized = false;
  let maximumTensorBytes = 0n;
  let maximumComputeMachineOps = 0n;
  let computeById = new Map();
  let computeIndexById = new Map();
  let layerNames = [];
  let layerOrdinalByName = new Map();
  let operationColours = new Map();
  let computeGroups = [];
  let maximumGroupNodeCount = 1;
  let maximumGroupMachineOps = 0n;
  let groupByCompute = [];
  const expandedGroups = new Set();
  let expandedLayers = new Set();
  const actualEdges = [];
  const adjacency = new Map();
  let tensorPlacement = new Map();
  const tensorBypassSide = new Map();
  let tensorRoles = new Map();
  let dtypeColours = new Map();
  const platformMemories = data.memory?.platform_memories || [];
  let memoryColours = new Map();
  let tensorMemories = new Map();
  ...
  function ensureGraphData() {
    if (graphDataInitialized) return;
    maximumTensorBytes = tensorList.reduce((maximum, tensor) => {
      const size = toBigInt(tensor.num_bytes);
      return size > maximum ? size : maximum;
    }, 0n);
    maximumComputeMachineOps = computeNodes.reduce((maximum, compute) => {
      const total = toBigInt(compute.machine_ops);
      return total > maximum ? total : maximum;
    }, 0n);
    computeById = new Map(computeNodes.map((node) => [node.id, node]));
    computeIndexById = new Map(
      computeNodes.map((node, index) => [node.id, index]),
    );
    layerNames = [...new Set(computeNodes.map((node) => node.layer))].sort(
      (left, right) =>
        layerNumber(left) - layerNumber(right) ||
        left.localeCompare(right, undefined, { numeric: true }),
    );
    layerOrdinalByName = new Map(
      layerNames.map((layer, index) => [layer, index]),
    );
    operationColours = new Map(
    ...
    maximumGroupNodeCount = Math.max(
      1,
      ...computeGroups.map((group) => group.members.length),
    );
    maximumGroupMachineOps = computeGroups.reduce((maximum, group) => {
    ...
    groupByCompute = new Array(computeNodes.length);
    for (const group of computeGroups) {
      for (const member of group.members) groupByCompute[member] = group;
    }
    expandedLayers = new Set(computeNodes.map((node) => node.layer));
    ...
    dtypeColours = stableDataTypeColours(
      tensorList.map((tensor) => tensor.dtype),
    );
    memoryColours = stableCategoryColours(
      platformMemories.map((memory) => memory.name),
    );
    tensorMemories = new Map(tensorList.map((tensor) => [tensor.id, []]));
  ...
  Object.assign(App, {
    timetableGraphDataInitialized: () => graphDataInitialized,
