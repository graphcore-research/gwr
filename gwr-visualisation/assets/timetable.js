...
(() => {
  ...
  function renderLayerSummary() {
    ...
    const useLayerTraffic =
      isAllFilter(peFilterValue()) &&
      isAllFilter(tensorFilterValue()) &&
      isAllFilter(memoryFilterValue());
    const metrics = visibleLayers.map((layer) => {
      const aggregate = aggregateLayer(layer);
      const context = useLayerTraffic
        ? null
        : contextSnapshot(layer.name, peFilterValue());
      return {
        layer,
        nodes: aggregate.computeNodes,
        ops: toBigInt(aggregate.machineOps.total),
        tensorCount: context?.tensors.length ?? Number(layer.tensor_count),
        read: context?.readBytes ?? toBigInt(layer.tensor_read_bytes),
        write: context?.writeBytes ?? toBigInt(layer.tensor_write_bytes),
    ...
    for (const metric of metrics) {
      ...
      <div class="comparison-heading">
        ...
        <span>${fmt.format(metric.activePes)} PEs · ${fmt.format(metric.tensorCount)} tensors</span>
      </div>
    ...
    if (layers.length > visibleLayers.length) {
      const status = document.createElement("p");
      status.className = "row-limit-status";
      status.textContent = `Showing ${fmt.format(visibleLayers.length)} of ${fmt.format(layers.length)} layers`;
