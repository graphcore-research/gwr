...
(() => {
  ...
  function layerOverviewMetric(layer) {
    ...
    const traffic = layerOverviewTraffic(
      layer,
    ...
    return {
      ...
      write: traffic.writeBytes,
      tensors: traffic.tensorCount,
  ...
  function renderLayerSummary() {
    ...
    if (metrics.length > LAYER_WINDOW_SIZE) {
      ...
      const slider = navigator.querySelector("input");
      const status = navigator.querySelector("span");
      slider.addEventListener("input", (event) => {
        const start = Number(event.target.value);
        const end = Math.min(start + LAYER_WINDOW_SIZE, metrics.length);
        status.textContent = `Displaying ${fmt.format(start + 1)}-${fmt.format(end)} of ${fmt.format(metrics.length)} layers`;
      });
