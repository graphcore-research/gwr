...
(() => {
  ...
  const MAX_RELATIONSHIP_SOURCES = 500;
  const MAX_RELATIONSHIP_EDGES = 5_000;

  function limitRelationshipModel(model) {
    const matchingEdges = model.edges.length;
    const total = model.edges.reduce((sum, edge) => sum + edge.value, 0n);
    const selectedSource = relationshipSelection().source;
    const edges = strongestEdges(
      model.edges,
    ...
    const sources = model.sources.filter((source) => sourceIds.has(source.id));
    const targets = model.targets.filter((target) => targetIds.has(target.id));
    return {
      ...model,
      sources,
      targets,
      ...
      total,
      matchingEdges,
      omittedSources: Math.max(model.sourceCount - sources.length, 0),
      omittedEdges: matchingEdges - edges.length,
    };
  }
  ...
  function computeRelationshipModel() {
    ...
    const layers = relationshipLayers();
    const visibleLayers = selectedWindow(
      layers,
  ...
  function peMemoryRelationshipModel() {
    ...
    const visiblePes = selectedWindow(
      pes,
      MAX_RELATIONSHIP_SOURCES,
      state.selectedPe?.name,
      (pe) => pe.name,
    );
    const sources = visiblePes.map((pe) => ({
      id: pe.name,
      label: pe.name,
      group: `PE row ${pe.row}`,
      order: [pe.row, pe.col],
  ...
  function tensorRelationshipLayer(tensor) {
    ...
    const productionLayers = connectionLayers(tensor.writes_by_pe);
    const layers = productionLayers.length
      ? productionLayers
      : connectionLayers(tensor.reads_by_pe);
  ...
  function tensorMemoryRelationshipModel() {
    const measure = relationshipControls.measure.value;
    const memories = relationshipMemories();
    const tensors = tensorsForContext();
    const visibleTensors = selectedWindow(
      tensors,
      MAX_RELATIONSHIP_SOURCES,
  ...
  function relationshipModel() {
    ...
    const model = limitRelationshipModel(
      builders[relationshipControls.mode.value](),
    );
  ...
  function renderRelationships() {
    ...
    ${model.omittedSources ? `<span>${fmt.format(model.omittedSources)} sources omitted</span>` : ""}
    ${model.omittedEdges ? `<span>${fmt.format(model.omittedEdges)} links omitted</span>` : ""}
