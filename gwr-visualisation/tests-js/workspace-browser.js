...
async function checkReport(browser, directory) {
  ...
  try {
    ...
    assert.equal(
      await page.evaluate(() =>
        window.GWR_VISUALISATION_APP.timetableGraphDataInitialized(),
      ),
      false,
    );
    await page.evaluate(() => {
    ...
    for (const name of ["layers", "compute", "memory", "tensors", "dataflow"]) {
      ...
      if (name === "layers") {
        const layerSlider = page.locator(
          ".workspace-primary .layer-window-navigator input",
        );
        if (await layerSlider.count()) {
          assert.equal(
            await layerSlider.evaluate((slider) => {
              slider.value = String(Number(slider.min) + Number(slider.step));
              slider.dispatchEvent(new Event("input", { bubbles: true }));
              return slider.isConnected;
            }),
            true,
            "Dragging the layer-window slider must not replace the active input",
          );
          await layerSlider.dispatchEvent("change");
          await settle();
          assert.match(
            await page
              .locator(".workspace-primary .layer-window-navigator span")
              .textContent(),
            /^Displaying 201-/,
          );
        }
    ...
    const overviewRendering = await navigator.evaluate((canvas) => {
      const model = window.GWR_VISUALISATION_APP.graphSelectionModel();
      const pixels = canvas
        .getContext("2d")
      ...
      return {
        finiteBounds: Object.values(model.bounds).every(Number.isFinite),
        finiteNodes: model.nodes.every((node) =>
          [node.x, node.y].every(Number.isFinite),
        ),
        paintedPixels,
      };
    });
    assert.equal(overviewRendering.finiteBounds, true);
    assert.equal(overviewRendering.finiteNodes, true);
    assert.ok(
      overviewRendering.paintedPixels > 0,
      "The graph overview must contain rendered pixels",
    );
