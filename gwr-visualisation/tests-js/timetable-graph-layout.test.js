...
test("keeps exactly separated decimal-sized tensors at finite positions", () => {
  const first = rectangle("first", 0, 0, 36, 189.538304);
  const second = rectangle("second", 0, 1, 36, 140.30758400000002);
  const tall = rectangle("tall", 1, 0, 36, 1528.2382079999998);
  const graph = model([first, second, tall], []);
  graph.computeLanes = ["unused"];

  layoutTimetableGraph(graph);

  assert.ok(Number.isFinite(first.y));
  assert.ok(Number.isFinite(second.y));
  assert.ok(Number.isFinite(graph.bounds.top));
  assert.ok(Number.isFinite(graph.bounds.bottom));
});
