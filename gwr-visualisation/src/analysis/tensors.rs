...
#[derive(Clone, Copy)]
struct TensorAccessRange {
    addr: u64,
    num_bytes: u64,
}

fn tensor_node_access(
    tensor_id: &str,
    node_id: &str,
    edge_index: Option<usize>,
    tensors_by_id: &BTreeMap<String, TensorSummary>,
...
fn compute_view_access(
    ...
    let config = tensor_configs_by_id.get(tensor_id)?;
    let views = node_views.get(compute_node)?;
    let relative_range = edge_index
        .and_then(|index| views.get(index)?.as_ref())
        .map_or(Some(0..tensor.num_bytes), |view| {
            view_physical_byte_range(config, view)
        })?;
    absolute_access(tensor.addr, relative_range)
}

fn memory_view_access(
    tensor_id: &str,
    tensors_by_id: &BTreeMap<String, TensorSummary>,
    tensor_configs_by_id: &BTreeMap<String, TensorConfigSection>,
    ...
    let tensor = tensors_by_id.get(tensor_id)?;
    let config = tensor_configs_by_id.get(tensor_id)?;
    let relative_range = memory_config?
        .view
        .as_ref()
        .map_or(Some(0..tensor.num_bytes), |view| {
            view_physical_byte_range(config, view)
...
fn view_physical_bytes(config: &TensorConfigSection, view: &TensorViewSection) -> Option<u64> {
    let range = view_physical_byte_range(config, view)?;
    range.end.checked_sub(range.start)
}

fn view_physical_byte_range(
    ...
    checked_dtype_byte_range(&config.dtype, offset, elements)
}

fn absolute_access(
    tensor_addr: u64,
    relative_range: std::ops::Range<u64>,
) -> Option<TensorAccessRange> {
    Some(TensorAccessRange {
        addr: tensor_addr.checked_add(relative_range.start)?,
        num_bytes: relative_range.end.checked_sub(relative_range.start)?,
    })
}
...
fn push_traffic(
    ...
    pe: Option<&str>,
    access: Option<TensorAccessRange>,
    layer: Option<&str>,
) {
    let pe = pe.unwrap_or("unassigned").to_string();
    let bytes = access.map_or(0, |access| access.num_bytes);
    if let Some(entry) = consumption_by_pe.iter_mut().find(|entry| entry.pe == pe) {
        ...
        if let Some(access) = access {
            entry.accesses.push(TensorTrafficAccess {
                addr: access.addr,
    ...
    consumption_by_pe.push(TensorPeConsumption {
        ...
        by_layer,
        accesses: access
            .map(|access| TensorTrafficAccess {
                addr: access.addr,
                num_bytes: access.num_bytes,
                layer: layer.map(str::to_string),
            })
