...
impl PeSummary {
    ...
    pub(crate) fn new(name: String, col: usize, row: usize) -> Self {
        Self {
            name,
            row: u64_from_usize(row),
            col: u64_from_usize(col),
            total_nodes: 0,
...
pub(crate) struct TensorTrafficAccess {
    ...
    pub(crate) ranges: Vec<TensorTrafficRange>,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
pub(crate) struct TensorTrafficRange {
    #[serde(
        serialize_with = "serialize_u64_as_string",
        deserialize_with = "deserialize_u64"
    )]
...
mod tests {
    ...
    fn round_trips_tensor_shapes_larger_than_wasm_usize() {
        ...
        assert_eq!(json["shape"][0], u64::from(u32::MAX) + 1);
