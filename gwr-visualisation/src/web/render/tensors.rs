...
use super::super::format::{bytes_u64, decimal, escape, hex, integer};
...
pub(super) fn render_memory_map(model: &AppModel, document: &Document) -> Result<(), JsValue> {
    ...
    html.push_str("</div>");
    let allocated = regions.iter().map(|region| region.allocated).sum::<u64>();
    write!(
        html,
...
pub(super) fn tensor_row(
    ...
    let left = layout.address.saturating_sub(region.start) as f64 / region.span() as f64 * 100.0;
    let width = (layout.bytes as f64 / region.span() as f64 * 100.0)
        .max(0.35)
        .min(100.0 - left);
