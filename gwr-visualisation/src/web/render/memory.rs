...
use super::super::format::{bytes, bytes_u64, decimal, escape, hex, integer};
use super::super::logic::{AddressRange, AppModel, TensorLayout, build_regions, range_union_bytes};
use super::set_html;
use super::tensors::tensor_row;
...
pub(super) fn render_summary(model: &AppModel, document: &Document) -> Result<(), JsValue> {
    ...
    let capacity = bytes_u64(totals.capacity);
    let allocated = bytes_u64(totals.allocated);
    let read = bytes_u64(totals.read);
    let written = bytes_u64(totals.write);
...
pub(super) fn render_overview(model: &AppModel, document: &Document) -> Result<(), JsValue> {
    ...
    for value in metrics {
        ...
        write!(
            ...
            is_selected,
            escape(&value.memory.name),
            bytes_u64(value.allocated),
            bytes_u64(value.read),
            bytes_u64(value.write),
            escape(&value.memory.name),
            escape(&value.memory.kind),
            integer(value.tensor_indices.len() as u64),
            comparison("Allocated", value.allocated as f64, format!("{} <em>{}%</em>", bytes_u64(value.allocated), decimal(value.allocated as f64 / capacity as f64 * 100.0, 3)), "allocated", maximum_allocated as f64, None),
...
pub(super) fn render_detail(model: &AppModel, document: &Document) -> Result<(), JsValue> {
    ...
    let mut html = format!(
        ...
        hex(selected
            .memory
            .base_addr
            .saturating_add(selected.memory.capacity_bytes)),
        bytes_u64(selected.allocated),
        bytes_u64(capacity),
    ...
    model
        ...
        .map(|memory| {
            ...
            for (index, tensor) in model.data.tensors.iter().enumerate() {
                ...
                let start = tensor.addr.max(memory.base_addr);
                ...
                result.read = result.read.saturating_add(traffic.read_bytes);
                result.write = result.write.saturating_add(traffic.write_bytes);
            }
            result.allocated = range_union_bytes(allocation_ranges);
            result
        })
        .collect()
}
...
) -> Vec<super::super::logic::MemoryRegion> {
    let layouts = metrics
        .tensor_indices
        .iter()
        .filter_map(|index| {
        ...
        .collect();
    build_regions(layouts, model.state.skip_memory_gaps)
}

fn totals(metrics: &[MemoryMetrics<'_>]) -> Totals {
    metrics.iter().fold(Totals::default(), |mut totals, value| {
        totals.capacity = totals.capacity.saturating_add(value.memory.capacity_bytes);
        totals.allocated = totals.allocated.saturating_add(value.allocated);
        totals.read = totals.read.saturating_add(value.read);
        totals.write = totals.write.saturating_add(value.write);
        totals
    })
}
...
fn overlap_bytes(left_start: u64, left_bytes: u64, right_start: u64, right_bytes: u64) -> u64 {
...
mod tests {
    ...
    #[test]
    fn overlap_is_limited_to_intersection() {
        assert_eq!(overlap_bytes(100, 40, 120, 100), 20);
        assert_eq!(overlap_bytes(100, 40, 200, 100), 0);
    }
}
