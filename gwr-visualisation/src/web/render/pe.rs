...
use super::super::state::{PeMeasure, PeMode, TrafficDirection, TrafficMeasure};
...
pub(super) fn render_overview(model: &AppModel, document: &Document) -> Result<(), JsValue> {
    ...
    if model.state.pe_mode == PeMode::Chart {
...
fn render_grid(model: &AppModel, document: &Document) -> Result<(), JsValue> {
    let (rows, cols) = model
        .grid_dimensions()
        .expect("grid mode is enabled only for safe dimensions");
    let measure = measure(model);
    let values = measure_values(model);
    let population = model.compute_population();
    ...
    for pe in &model.data.pes {
        let row = usize::try_from(pe.row).expect("safe PE grid row fits in usize");
        let col = usize::try_from(pe.col).expect("safe PE grid column fits in usize");
        by_coordinate.entry((row, col)).or_default().push(pe);
    }
...
fn measure_values(model: &AppModel) -> HashMap<String, f64> {
    match &model.state.pe_measure {
        PeMeasure::ComputeNodes => compute_node_values(model),
        PeMeasure::MachineOperation(operation) => model
            .data
            .pes
            .iter()
            .map(|pe| {
            ...
            .collect(),
        PeMeasure::MachineOps => model
            .data
            .pes
            .iter()
            .map(|pe| (pe.name.clone(), model.machine_ops_for_pe(pe).total as f64))
            .collect(),
        PeMeasure::Data(measure) => traffic_values(model, model.filtered_tensors(), *measure),
        PeMeasure::SelectedTensor(direction) => {
            let tensors = model.selected_tensor().into_iter().collect::<Vec<_>>();
            traffic_values(model, tensors, TrafficMeasure::from(*direction))
        }
        PeMeasure::Overlay(name) => model
            .data
            .pes
            .iter()
...
fn traffic_values(
    ...
    for tensor in tensors {
        ...
        if matches!(measure, TrafficMeasure::Read | TrafficMeasure::Total) {
            add_connections(&mut values, &traffic.reads);
        }
        if matches!(measure, TrafficMeasure::Write | TrafficMeasure::Total) {
...
fn measure(model: &AppModel) -> Measure<'_> {
    ...
    match &model.state.pe_measure {
        ...
        PeMeasure::ComputeNodes => Measure {
            group: "Compute allocation",
        ...
        PeMeasure::Data(TrafficMeasure::Total) => Measure {
            group: "Data",
            label: "Total".into(),
            colour: "--activity",
            unit: "bytes",
        ...
        PeMeasure::Data(TrafficMeasure::Read) => Measure {
            group: "Data",
            label: "Read".into(),
            colour: "--read",
            unit: "bytes",
        ...
        PeMeasure::Data(TrafficMeasure::Write) => Measure {
            group: "Data",
            label: "Written".into(),
            colour: "--write",
            unit: "bytes",
        ...
        PeMeasure::SelectedTensor(TrafficDirection::Read) => Measure {
        ...
        PeMeasure::SelectedTensor(TrafficDirection::Write) => Measure {
            group: "Selected tensor",
        ...
        PeMeasure::MachineOperation(operation) => machine_operation_measure(model, operation),
        PeMeasure::Overlay(name) => overlay_measure(model, name),
    }
}

fn machine_operation_measure<'a>(model: &'a AppModel, operation: &str) -> Measure<'a> {
...
fn overlay_measure<'a>(model: &'a AppModel, name: &str) -> Measure<'a> {
...
fn set_overview_mode(model: &AppModel, document: &Document) -> Result<(), JsValue> {
    element(document, "pe-overview-chart")?
        ...
        .set_hidden(model.state.pe_mode != PeMode::Chart);
    element(document, "pe-overview-grid")?
        .dyn_into::<HtmlElement>()?
        .set_hidden(model.state.pe_mode != PeMode::Grid);
