...
pub(crate) fn initialize_controls(model: &AppModel, document: &Document) -> Result<(), JsValue> {
    ...
    select(document, "pe-overview-measure")?.set_value(&model.state.pe_measure);
...
pub(crate) fn update_filter_summaries(
    ...
    for (kind, noun, id) in [
        ...
        let text = if filter.is_all() {
            format!("All {}", integer(model.filter_value_count(kind) as u64))
...
pub(crate) fn update_relationship_measure_options(
    ...
    let values: Vec<(&str, String)> = match model.state.relationship_mode.as_str() {
        "compute" => std::iter::once(("machine-ops", "Machine ops".into()))
    ...
    if values
        .iter()
        .any(|(value, _)| *value == model.state.relationship_measure)
    {
        select.set_value(&model.state.relationship_measure);
    }
    Ok(())
}
...
fn render_global_stats(model: &AppModel, document: &Document) -> Result<(), JsValue> {
    ...
    set_text(
        document,
        "stat-compute",
        &format!("{} compute nodes", integer(summary.compute_nodes as u64)),
    )?;
    set_text(document, "stat-tensors", &integer(summary.tensors as u64))?;
    ...
    set_text(document, "stat-edges", &integer(summary.edges as u64))?;
    set_text(document, "stat-pes", &integer(summary.active_pes as u64))
}
...
fn render_layer_summary(model: &AppModel, document: &Document) -> Result<(), JsValue> {
    ...
    for index in visible_layer_indices(model, &metrics) {
        ...
        write!(
            ...
            selected,
            escape(&layer.name),
            integer(aggregate.compute_nodes as u64),
            ...
            integer(aggregate.tensor_count as u64),
...
fn layer_metric(model: &AppModel, layer: &LayerSummary) -> LayerAggregate {
    ...
    aggregate.tensor_count = context.tensor_indices.len();
...
fn layer_pe_metric(
    ...
    LayerAggregate {
        ...
        tensor_count: context.tensor_indices.len(),
...
fn compute_nodes_markup(model: &AppModel, total: usize, by_op: &BTreeMap<String, usize>) -> String {
    let entries = model
        .data
        .ops
        .iter()
        .map(|operation| {
            (
                operation.as_str(),
                count(*by_op.get(operation).unwrap_or(&0) as f64),
            )
        })
        .collect::<Vec<_>>();
    metric_breakdown("Compute nodes", integer(total as u64), &entries)
}
...
// Implemented below in focused view sections.
