...
use crate::model::{
    ...
    TensorTrafficAccess, VisualisationData,
};
...
impl AppModel {
    ...
    pub(crate) fn filter_value_count(&self, kind: EntityKind) -> usize {
        if kind == EntityKind::Tensor && self.data.tensors.is_empty() {
            return self.data.summary.tensor_nodes;
        }
        self.state.filter(kind).values().len()
    }
    ...
    pub(crate) fn filtered_summary(&self) -> FilteredSummary {
        ...
        result.tensors = context.tensor_indices.len();
    ...
    pub(crate) fn compute_nodes_for_pe(&self, pe_name: &str) -> (usize, BTreeMap<String, usize>) {
        let mut nodes = 0;
    ...
    pub(crate) fn set_pe_mode(&mut self, requested: &str) {
        ...
        self.state.pe_mode = match requested {
            "chart" => "chart",
            "grid" if !grid_is_safe(rows, cols) => "chart",
            _ => "grid",
        }
        .into();
    }
    ...
    pub(crate) fn memory_regions(&self) -> Vec<MemoryRegion> {
        ...
        layouts.sort_by_key(|layout| {
            (
                layout.address,
                self.data.tensors[layout.tensor_index].id.as_str(),
            )
        });
        build_regions(layouts, self.state.skip_memory_gaps)
    }
...
fn grid_is_safe(rows: usize, cols: usize) -> bool {
    rows > 0
        && cols > 0
        && rows
            .checked_mul(cols)
            .is_some_and(|cells| cells <= MAX_PE_GRID_CELLS)
}
...
fn add_ops(total: &mut BTreeMap<String, usize>, values: &BTreeMap<String, usize>) {
...
fn sum_ops<'a>(
    values: impl Iterator<Item = &'a BTreeMap<String, usize>>,
...
fn connection_traffic(
    ...
    connection
        ...
        .fold((0_u64, 0_usize), |(bytes, edges), access| {
            let selected_bytes = selected_access_bytes(access, memory_ranges);
            if selected_bytes == 0 {
                (bytes, edges)
            } else {
                (
...
fn selected_access_bytes(
    ...
    let access_range = AddressRange::new(access.addr, access.num_bytes);
    let Some(memory_ranges) = memory_ranges else {
        return access.num_bytes;
    };
    range_union_bytes(
        memory_ranges
            .iter()
...
fn clipped_range(
    ...
    let end = left_start
        ...
        .min(right_start.saturating_add(right_bytes));
    (end > start).then_some((start, end - start))
}

pub(crate) fn build_regions(layouts: Vec<TensorLayout>, skip_gaps: bool) -> Vec<MemoryRegion> {
    let largest = layouts.iter().map(|layout| layout.bytes).max().unwrap_or(1);
    let total = layouts
        .iter()
        .fold(0_u64, |sum, layout| sum.saturating_add(layout.bytes));
    let threshold = skip_gaps.then_some(largest.max(total / 64).max(4096));
    let mut regions: Vec<MemoryRegion> = Vec::new();
    for layout in layouts {
        let end = layout.address.saturating_add(layout.bytes);
        let gap = regions
            .last()
            .map_or(0, |region| layout.address.saturating_sub(region.end));
        if regions.is_empty() || threshold.is_some_and(|threshold| gap > threshold) {
            regions.push(MemoryRegion {
                start: layout.address,
                end,
                gap_before: usize::from(!regions.is_empty()) as u64 * gap,
                allocated: layout.bytes,
                tensors: vec![layout],
            });
        } else if let Some(region) = regions.last_mut() {
            region.end = region.end.max(end);
            region.allocated = region.allocated.saturating_add(layout.bytes);
...
mod tests {
    ...
    use super::{
        AddressRange, AppModel, TensorLayout, build_regions, connection_traffic, grid_is_safe,
        range_union_bytes,
    };
    use crate::analysis::summarize;
    use crate::model::{TensorPeConsumption, TensorTrafficAccess};
    ...
    fn rejects_unsafe_pe_grid_dimensions() {
        ...
        assert!(!grid_is_safe(usize::MAX, 2));
    }
