...
use super::render;
use super::state::{
    EntityKind, PeMeasure, PeMode, RelationshipMeasure, RelationshipMode, TrafficDirection,
    TrafficMeasure,
};
...
impl Application {
    ...
    fn apply_preset_modes(&mut self, name: &str) -> Result<(), JsValue> {
        let previous_relationship = self.model.state.relationship_mode;
        let previous_pe_measure = self.model.state.pe_measure.clone();
        let previous_pe_mode = self.model.state.pe_mode;
        match name {
            "compute" => {
                self.model.state.relationship_mode = RelationshipMode::Compute;
                self.model.state.pe_measure = PeMeasure::MachineOps;
                self.model.set_pe_mode(PeMode::Grid);
            }
            "memory" => {
                self.model.state.relationship_mode = RelationshipMode::LayerMemory;
                self.model.state.pe_measure = PeMeasure::Data(TrafficMeasure::Total);
                self.model.set_pe_mode(PeMode::Chart);
            }
            "tensor" => {
                self.model.state.pe_measure = PeMeasure::SelectedTensor(TrafficDirection::Read);
                self.model.set_pe_mode(PeMode::Grid);
            }
            _ => return Ok(()),
        }
        select(&self.document, "relationship-mode")?
            .set_value(self.model.state.relationship_mode.name());
        render::update_relationship_measure_options(&self.model, &self.document)?;
        self.model.state.relationship_measure =
            RelationshipMeasure::parse(&select(&self.document, "relationship-measure")?.value())
                .unwrap_or(RelationshipMeasure::MachineOps);
        select(&self.document, "pe-overview-measure")?
            .set_value(&self.model.state.pe_measure.name());
    ...
    fn handle_click(&mut self, event: Event) -> Result<(), JsValue> {
        ...
        if let Some(button) = target.closest("[data-pe-overview-mode]")? {
            ...
            if let Some(mode) = PeMode::parse(&requested) {
                self.model.set_pe_mode(mode);
            }
            self.mark_panels_dirty(&["pe-grid"]);
    ...
    fn handle_change(&mut self, event: Event) -> Result<(), JsValue> {
        ...
        match target.id().as_str() {
            "relationship-mode" => {
                let value = target.dyn_into::<HtmlSelectElement>()?.value();
                self.model.state.relationship_mode = RelationshipMode::parse(&value)
                    .ok_or_else(|| JsValue::from_str("Unknown relationship mode"))?;
                render::update_relationship_measure_options(&self.model, &self.document)?;
                let value = select(&self.document, "relationship-measure")?.value();
                self.model.state.relationship_measure = RelationshipMeasure::parse(&value)
                    .ok_or_else(|| JsValue::from_str("Unknown relationship measure"))?;
                self.mark_panels_dirty(&["relationships"]);
                self.render_dirty()
            }
            ...
            "pe-overview-measure" => {
                let value = target.dyn_into::<HtmlSelectElement>()?.value();
                self.model.state.pe_measure = PeMeasure::parse(&value)
                    .ok_or_else(|| JsValue::from_str("Unknown PE measure"))?;
                self.mark_panels_dirty(&["pe-grid"]);
                self.render_dirty()
            }
            "skip-memory-gaps" => {
    ...
    fn handle_pointer_up(&mut self, _event: Event) -> Result<(), JsValue> {
        self.workspace.save(&self.document);
        Ok(())
    }
}
...
fn bind_events(document: &Document) -> Result<(), JsValue> {
    ...
    listen(document, "pointerup", false, Application::handle_pointer_up)
}
