...
pub(crate) fn count(value: f64) -> String {
    if value.abs() < 10_000.0 {
        return signed_integer(value.round());
    }
    ...
    signed_integer(value.round())
}

fn signed_integer(value: f64) -> String {
    if value < 0.0 {
        format!("-{}", integer(value.abs() as u128))
    } else {
...
pub(crate) fn bytes_u128(value: u128) -> String {
    let Ok(value) = u64::try_from(value) else {
        const EIB: u128 = 1 << 60;
        let rounded = (value + EIB / 2) / EIB;
        return format!("{} EiB", integer(rounded));
    };
    bytes_u64(value)
}
...
#[cfg(test)]
mod tests {
    use super::{bytes, bytes_u64, bytes_u128, count, count_u64, escape, hex_u128, integer};

    #[test]
    fn formats_report_values() {
        assert_eq!(integer(1_234_567_u64), "1,234,567");
        assert_eq!(count(1_250_000.0), "1.25M");
        assert_eq!(count(-1_536.0), "-1,536");
        ...
        assert_eq!(bytes_u128(u128::from(u64::MAX) + 1), "16 EiB");
        assert_eq!(hex_u128(u128::from(u64::MAX) + 1), "0x10000000000000000");
        assert_eq!(
            escape("<tensor & \"name\">"),
            "&lt;tensor &amp; &quot;name&quot;&gt;"
        );
