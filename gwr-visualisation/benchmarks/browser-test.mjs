...
try {
  for (const adapter of browserAdapters(config)) {
    try {
      ...
      const regression = await adapter.withSession(regressionUrl, (session) =>
        exerciseRegressionReport(session),
      );
...
async function exerciseRegressionReport(session) {
  ...
  await click(session, "#tensor-filter-none");
  ...
  await click(session, "#tensor-filter-all");
