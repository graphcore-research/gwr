...
frontend and steam-spotter having loaded a binary trace file.
...
The first step is to create a binary trace by running a STEAM-based simulation.
For example:
...
### Load binary in steam-spotter

`steam-spotter` is a utility for reading trace files but will also open a port
for this frontend to interact with:

```bash
cargo run --release --bin steam-spotter -- --bin trace.bin
```
...
In the `force_tree` view you can select nodes and `steam-spotter` will be
