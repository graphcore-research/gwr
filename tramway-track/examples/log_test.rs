...
//! Basic test case using the tramway_track library
//!
...
//!
//! ```
//!   capnp convert packed:text tramway-track/schemas/tramway_track.capnp Event < foo.bin
//! ```
...
use tramway_track::tracker::{CapnProtoTracker, EntityManager, TextTracker, Tracker};
use tramway_track::{Writer, create_and_track_id, debug, info, trace};
...
fn load(
    ...
    address: &mut u32,
    value: &mut u64,
    tx_id: &mpsc::Sender<(u64, tramway_track::Id)>,
) {
...
fn main() -> io::Result<()> {
    ...
    let default_entity_level = tramway_track::str_to_level(&config.log_level.unwrap());
    ...
    let (tx_id, rx_id) = mpsc::channel::<(u64, tramway_track::Id)>();
