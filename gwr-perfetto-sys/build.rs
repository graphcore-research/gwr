...
fn main() {
    ...
    assert!(
        ...
        "{}",
        &format!(
            "Perfetto source submodule does not appear to have been checked out. Please run `git {}`.",
            get_git_args().join(" ")
        )
    );
}
...
fn submodule_is_updated() -> bool {
    let Ok(mut directory_entries) = read_dir(submodule_path::PERFETTO_SOURCE) else {
        return false;
    };
