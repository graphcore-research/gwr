...
When `mdbook build` is invoked the generated output is written to the `book`
directory. The build system also writes to the `doctest_cache` and
`rustdoc_cache` directories.
