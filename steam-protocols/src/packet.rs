...
macro_rules! build_packet_type {
    ($protocol:ident, $packet_type:ident, $types:ident, $pkt_type:ident; $($field:ident : $type:ty, $bits:expr, $scale:expr),+ $(,)*) => {
        ...
        impl [< $packet_type  >] {
            ...
            pub fn new(created_by: &std::sync::Arc<steam_track::entity::Entity>, cfg: &[< $packet_type Cfg >]) -> Self {
                let pkt = Self {
                ...
                steam_track::create!(created_by ; pkt, pkt.total_bytes(), pkt.req_type() as i8);
                pkt
            }
        ...
        impl steam_engine::traits::TotalBytes for [< $packet_type  >] {
        ...
        impl steam_engine::traits::SimObject for [< $packet_type  >] {}
