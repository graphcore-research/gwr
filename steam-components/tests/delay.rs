...
fn put_get() {
    ...
    let mut tx = OutPort::new(engine.top().clone(), "tb_tx");
    ...
    let rx = InPort::new(engine.top().clone());
...
fn source_sink() {
    ...
    let delay = Delay::new(engine.top(), "delay", clock.clone(), spawner, DELAY);
