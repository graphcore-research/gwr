...
//!
//! fn build_store(parent: &Arc<Entity>, spawner: Spawner) -> Store<i32> {
...
//!     //   - a capacity.
//!     let store = Store::new(parent, "store", spawner, 5);
//!     store
//! }
//! ```
//!
...
//! use steam_track::entity::Entity;
//!
//! fn build_store_with_panic(parent: &Arc<Entity>, spawner: Spawner) -> Store<i32> {
//!     // Create a store that panics on overflow. Use `new()` as before,
//!     // then call `set_panic_on_overflow()` on the resulting struct.
//!     let store = Store::new(parent, "store_panic", spawner, 5).set_panic_on_overflow();
//!     store
//! }
//! ```
...
//! [Sink](crate::sinks::Sink) to take the data out of the store.
//!
//! ```rust
//! use std::sync::Arc;
...
//! let source = Source::new(engine.top(), "source", option_box_repeat!(1 ; 10));
//! // Create the store - its type will be derived from the connections to its ports.
//! let store = Store::new(engine.top(), "store", spawner, 5);
...
//! run_simulation!(engine; [source, store, sink]);
//! ```
    ...
    T: SimObject,
{
    ...
    pub fn new(parent: &Arc<Entity>, name: &str, spawner: Spawner, capacity: usize) -> Self {
        ...
        Self {
            entity: entity.clone(),
            spawner,
            state: Rc::new(State::new(entity, capacity)),
        }
    }
