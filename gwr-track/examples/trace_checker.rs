...
//! Read and check a Cap'n Proto trace file created using gwr_track
//!
//! Checks:
...
impl Checker {
    fn new(info_ids: &[u64]) -> Self {
        ...
        let root = Node::new(gwr_track::NO_ID, gwr_track::ROOT);
        let no_id = Node::new(gwr_track::NO_ID, gwr_track::NO_ID);

        // It is always ok to use these special identifiers from gwr_track
        checker.active_ids.insert(gwr_track::ROOT, root);
        checker.active_ids.insert(gwr_track::NO_ID, no_id);
    ...
    fn destroy_id(&mut self, id: Id) {
        if id == gwr_track::ROOT || id == gwr_track::NO_ID {
    ...
    fn end_checks(&self) {
        let still_active = self
            .active_ids
            .iter()
            .filter(|&(k, _v)| *k != gwr_track::NO_ID && *k != gwr_track::ROOT);
