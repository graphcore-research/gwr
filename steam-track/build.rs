...
fn try_default_include_dir_env_var() -> String {
    let default = match std::env::consts::OS {
        "macos" if std::env::consts::ARCH == "aarch64" => {
            "/opt/homebrew/Cellar/capnp/1.1.0/include/capnp"
        }
        "macos" if std::env::consts::ARCH == "x86_64" => {
            "/usr/local/homebrew/Cellar/capnp/1.1.0/include/capnp"
        }
        _ => panic!(
...
fn get_include_dir_env_var() -> PathBuf {
    PathBuf::from(
        env::var(CAPNP_INC_DIR_ENV_VAR).unwrap_or_else(|_| try_default_include_dir_env_var()),
    )
}
...
fn compile_schemas(schemas: Vec<PathBuf>) {
    ...
    command.import_path(get_include_dir_env_var());
