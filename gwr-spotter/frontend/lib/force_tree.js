...
((app, d3) => {
  ...
  function force_tree_fill(d) {
    if (d.data.capacity > 0) {
      return app.fullnessColor(app.fullnessRatio(d.data, d.data.capacity));
    }
  ...
  function force_tree_title(d) {
    if (d.data.capacity > 0) {
      ...
      const percentage = d3.format(".1%")(
        app.fullnessRatio(d.data, d.data.capacity),
      );
  ...
  function force_tree(serverUrl, data, connections, max_depth) {
    ...
    var chartDiv = document.getElementById(app.chartElement);
    var width = Math.max(600, chartDiv.clientWidth);
    var height = Math.max(400, chartDiv.clientHeight - app.buttonBarPadding);
    ...
    links = links.filter(
      (link) => Math.min(link.source.data.depth, link.target.data.depth) > 0,
    );

    // Assign some properties to the links depending on their depth in the hierarchy.
    links.forEach(function (link) {
      const min_depth = Math.min(
        link.source.data.depth,
        link.target.data.depth,
      );
    ...
    node.append("title").text(force_tree_title);

    // When clicking on a block highlight it by setting the "selected" class
    node.on("click", (d) =>
      app.selectAndSend(serverUrl, svg, d.target.id, d.target.__data__.data.id),
    );
  ...
  Object.assign(app, {
    forceTree: force_tree,
    updateForceTreeFullness: update_force_tree_fullness,
  });
