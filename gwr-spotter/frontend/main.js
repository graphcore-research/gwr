...
((app, d3) => {
  ...
  function add_entity(depth, nodes, path_remaining, full_name, id, capacities) {
    ...
    let children = Object.hasOwn(node, "children") ? node.children : [];
    children = add_entity(
      depth + 1,
      children,
      path_remaining,
    ...
    if (num_children > 0) {
      ...
      node.children.forEach(function (child, i) {
        ...
        set_initial_xy(
          child,
          child.initial_x,
          child.initial_x + x_step,
          child.initial_y,
          child.initial_y + y_step,
        );
      });
    }
  }
  ...
  class Renderer {
    ...
    _createGuiElements() {
      const plotTypes = new Map()
        .set("sunburst", app.sunburst)
        .set("force_tree", app.forceTree)
        .set("tree_map", app.treeMap)
        .set("radial_tidy_tree", app.radialTidyTree);
    ...
    set_fullnesses(fullnesses) {
      ...
      if (this.entities != null) {
        ...
        if (this.controls.plot.value == "tree_map") {
          app.updateTreeMapFullness(this.entities);
        } else if (this.controls.plot.value == "force_tree") {
          app.updateForceTreeFullness(this.entities);
        }
      }
    }
    ...
    _render_position(position) {
      ...
      this.positionLabel.text(
        `${line} / ${maxLine} @ ${position.time.toFixed(1)}ns`,
      );
    }
    ...
    _seek(line) {
      ...
      d3.text(this.serverUrl + `/seek/${line}`).catch(function (error) {
        console.log(error);
      });
    }
    ...
    render() {
      ...
      d3.select(`#${app.chartElement}`).selectAll("svg").remove();
  ...
  Object.assign(app, {
    Renderer,
    fullnessColor: fullness_color,
    fullnessRatio: fullness_ratio,
    href,
    parseCapacities: parse_capacities,
    parseConnections: parse_connections,
    parseEntities: parse_entities,
    url,
  });
})(window.GWR_SPOTTER || (window.GWR_SPOTTER = {}), window.d3);
