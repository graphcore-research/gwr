...
fn write_recipe(
    ...
    // command-line
    let mut app = gwr_terminus::writer::app::App::new(history_path, "", false);
...
fn start_write_recipe_tui(history: &PathBuf, recipes_folder: &str) -> Result<()> {
    ...
    while app.is_running() {
        ...
        if let Event::Key(key_event) = event::read()? {
            gwr_terminus::writer::handler::handle_key_event(key_event, &mut app);
        }
    }
...
fn start_run_recipe_tui(recipes_folder: &str, tmp_root: &Path, keep_tmp: bool) -> Result<()> {
    ...
    while app.is_running() {
        ...
        // Handle events.
        if let Event::Key(key_event) = event::read()? {
            gwr_terminus::runner::handler::handle_key_event(key_event, &mut app);
        }
    }
