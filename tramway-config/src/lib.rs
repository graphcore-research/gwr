...
//! would be `TRAMWAY_LOG_LEVEL`, `TRAMWAY_ENABLE_TRACE`, `TRAMWAY_LOG_FILE`,
//! and `TRAMWAY_CONF_FILE`.
//!
...
//! * `RUSTFLAGS="-Zmacro-backtrace" cargo +nightly run -p tramway-track
//!   --example log_test`
//! * `cargo expand -p tramway-track --example log_test`.
...
/// ## Crates
///
/// Applications using ths macro must add `tramway-config`, `clap` (with the
...
/// tramway-config = { path = "../tramway-config" }
/// ```
///
/// ## Use statements
///
/// Before the [macro@multi_source_config] attribute can be used the
/// `use tramway_config::multi_source_config;` statement must be present. Use
...
/// `TRAMWAY_`, with [Figment] handling any case conversions as necessary.
...
/// # }
/// ```
/// The `log_level` field could be set via the `TRAMWAY_LOG_LEVEL` environment
/// variable.
///
/// # Configuration files
///
    ...
    fn defaults_and_alternative_and_extra_conf_files() {
        let run = String::from_utf8(
            Command::new("cargo")
                ...
                .args([
                    "--conf-file",
                    "tramway-config/examples/runtime_conf_file.toml",
                ])
                .current_dir(get_workspace_dir())
                .output()
                .unwrap()
