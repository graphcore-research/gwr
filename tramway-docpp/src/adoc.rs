...
//! Process the `adoc!` documentation proc_macro.
...
use tramway_doc_builder::helpers::{CommandDescriptor, env_doc_builder, handle_error};
...
pub fn process(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    handle_error(|| {
        ...
        let output = Command::new("asciidoctor")
            .arg("-b") // Request HTML output
            .arg("html5")
            .arg("-e") // Use embedded mode
            .arg(tmp.path())
            .arg("-o") // Write output to stdout
            .arg("-")
            .output()
            .expect("\n\n**Failed to run asciidoctor - make sure it is on the path**");
