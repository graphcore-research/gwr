...
use gwr_models::processing_element::operators::MachineOp;
...
fn stats_include_machine_ops_for_compute_nodes() {
    ...
    assert_eq!(stats.machine_ops.get(&MachineOp::Add), Some(&26));
    assert_eq!(stats.machine_ops.get(&MachineOp::Mul), Some(&24));
    assert_eq!(stats.machine_ops.get(&MachineOp::Compare), None);
    ...
    assert_eq!(compute_nodes[0].machine_ops.get(&MachineOp::Add), Some(&10));
    assert_eq!(compute_nodes[0].machine_ops.get(&MachineOp::Mul), None);
    ...
    assert_eq!(compute_nodes[1].machine_ops.get(&MachineOp::Add), Some(&16));
    assert_eq!(compute_nodes[1].machine_ops.get(&MachineOp::Mul), Some(&24));
