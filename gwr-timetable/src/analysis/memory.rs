...
impl BandwidthGraph {
    ...
    #[must_use]
    pub fn cache_names_on_paths(&self, from: &str, to: &str) -> BTreeSet<String> {
        let mut caches = BTreeSet::new();
        let mut visited = BTreeSet::new();
        self.collect_caches_on_paths(from, to, &mut visited, &mut caches);
        caches
    }

    fn collect_caches_on_paths(
        &self,
        current: &str,
        target: &str,
        visited: &mut BTreeSet<String>,
        caches: &mut BTreeSet<String>,
    ) -> bool {
        if current == target {
            return true;
        }
        ...
        if let Some(edges) = self.edges.get(current) {
            for (next, _) in edges {
                if self.collect_caches_on_paths(next, target, visited, caches) {
                    reaches_target = true;
                }
            }
        }
        ...
        if reaches_target && let Some(cache_name) = current.strip_prefix("cache:") {
            caches.insert(cache_name.to_string());
        }
        reaches_target
    }
}
