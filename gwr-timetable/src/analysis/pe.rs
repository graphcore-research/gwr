...
pub fn compute_node_rooflines(
    ...
    for analysis in compute_nodes {
        ...
        for (memory_name, bytes) in &bytes_by_memory {
            ...
            memory_ticks += ticks_for_bytes(transferred_bytes, bandwidth);
        }
...
fn process_memory_events(memory_name: &str, events: &[MemoryEvent]) -> MemoryContentionAnalysis {
    ...
    while event_idx < events.len() {
        ...
        let total_requested_fraction = active
            .values()
        ...
        for (node_idx, (requested_fraction, memory_fraction)) in &active {
            let adjusted_ticks = memory_fraction * duration_ticks * slowdown;
            debug!(
                "process_memory_events: memory='{memory_name}', node_idx={node_idx}, requested_fraction={requested_fraction:.3}, memory fraction={memory_fraction:.3}, adjusted_ticks={adjusted_ticks:.3}",
            );
            *analysis
