...
//! PE-side roofline costing, approximate scheduling, and critical-path helpers.
//!
//! New callers should prefer [`crate::analysis::roofline::RooflineAnalyzer`]
//! rather than orchestrating these pieces directly.
...
#[must_use]
fn transferred_bytes_for_memory_access(
    payload_bytes: usize,
    lsu_access_bytes: usize,
    ...
    let chunk_size = lsu_access_bytes.max(1);
    let num_chunks = payload_bytes.div_ceil(chunk_size);
    payload_bytes + num_chunks * overhead_size_bytes * 2
}
...
pub fn compute_node_rooflines(
    ...
    for analysis in compute_nodes {
        ...
        let memory_ticks_by_memory = base_memory_ticks_by_memory(
            platform_pe,
            pe_name,
            &bytes_by_memory,
            bandwidth_graph,
        ...
        let memory_ticks = memory_ticks_by_memory.values().sum();
...
pub fn aggregate_pe_rooflines(
    ...
    let mut machine_ops_by_pe: HashMap<String, MachineOpCounts> = HashMap::new();
    ...
    for node in node_rooflines {
        ...
        machine_ops_by_pe
            .entry(pe_name.clone())
            .or_default()
            .add_assign(node.analysis.machine_ops);
...
fn base_memory_ticks_by_memory(
    platform_pe: &gwr_models::processing_element::ProcessingElement,
    pe_name: &str,
    ...
    let lsu_access_bytes = platform_pe.lsu_access_bytes();
    let overhead_size_bytes = platform_pe.overhead_size_bytes();
    for (memory_name, bytes) in bytes_by_memory {
        ...
        let transferred_bytes =
            transferred_bytes_for_memory_access(*bytes, lsu_access_bytes, overhead_size_bytes);
        let ticks = ticks_for_bytes(transferred_bytes, bandwidth);
        ticks_by_memory.insert(memory_name.clone(), ticks);
    }
    Ok(ticks_by_memory)
}

fn topological_compute_node_order(
    node_rooflines: &[ComputeNodeRoofline],
    ...
    if ordered.len() != node_rooflines.len() {
        return Err(SimError(format!(
            "Compute graph contains a cycle or dangling dependency: ordered {} of {} nodes",
            ordered.len(),
            node_rooflines.len()
        )));
    }
...
fn build_activity_models(
    ...
    for node in node_rooflines {
        ...
        let platform_pe = platform.pe(&pe_name)?;
        let base_memory_ticks_by_memory = base_memory_ticks_by_memory(
            platform_pe,
            &pe_name,
            &node.bytes_by_memory,
            graph,
            widest_path_cache,
        )?;
...
fn build_schedule_context(
    node_rooflines: &[ComputeNodeRoofline],
) -> Result<ScheduleContext, SimError> {
...
fn build_memory_events(memory_name: &str, activities: &[ActivityView<'_>]) -> Vec<MemoryEvent> {
    ...
    for activity in activities {
        ...
        debug!(
            "build_memory_events: node='{}' (node_idx={}) memory='{}' start={:.3} end={:.3} duration={:.3} base_ticks={:.3} requested={:.3}",
            activity.model.node.analysis.id,
            activity.model.node.analysis.node_idx,
            memory_name,
...
fn process_memory_events(memory_name: &str, events: &[MemoryEvent]) -> MemoryContentionAnalysis {
    ...
    let mut active: HashMap<usize, f64> = HashMap::new();
    let mut event_idx = 0;
    ...
    while event_idx < events.len() {
        ...
        let total_requested_fraction = active.values().sum::<f64>();
        let slowdown = total_requested_fraction.max(1.0);
        let active_node_indices = active.keys().copied().collect::<Vec<_>>();
        debug!(
            "process_memory_events: memory='{memory_name}', window start={time_ticks:.3}, end={next_time_ticks:.3}, duration={duration_ticks:.3}, requested={total_requested_fraction:.3}, slowdown={slowdown:.3}, active={active_node_indices:?}",
        );
        for (node_idx, requested_fraction) in &active {
            let adjusted_ticks = requested_fraction * duration_ticks * slowdown;
            debug!(
                "process_memory_events: memory='{memory_name}', node_idx={node_idx}, requested_fraction={requested_fraction:.3}, adjusted_ticks={adjusted_ticks:.3}",
            );
            *analysis
        ...
        analysis.windows.push(MemoryContentionWindow {
            ...
            requested_fraction: total_requested_fraction,
            active_node_indices,
        });
    }
...
fn apply_memory_contention_from_analysis(
    ...
    for (model, state) in models.iter().zip(states.iter_mut()) {
        ...
        for memory_name in model.base_memory_ticks_by_memory.keys() {
            ...
            let adjusted_ticks = adjusted_ticks.max(base_ticks).max(previous_ticks);
            debug!(
...
pub fn schedule_pe_activities(
    ...
    let models =
        build_activity_models(platform, node_rooflines, bandwidth_graph, widest_path_cache)?;
    ...
    if timings {
        eprintln!("timing: schedule.iterations {iteration_count}");
    }
...
pub fn critical_path_analysis(
    node_rooflines: &[ComputeNodeRoofline],
) -> Result<CriticalPathAnalysis, SimError> {
    ...
    if best_finish.len() != node_rooflines.len() {
        return Err(SimError(format!(
            "Compute graph contains a cycle or dangling dependency: analysed {} of {} nodes",
            best_finish.len(),
            node_rooflines.len()
        )));
    }
    ...
    let Some((end_node_idx, total_ticks)) = best_finish
        ...
        return Ok(CriticalPathAnalysis {
            total_ticks: 0.0,
