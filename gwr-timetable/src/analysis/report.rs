...
use crate::analysis::cache::{CacheModel, CacheSharingReport};
...
pub fn print_cache_sharing_report(report: &CacheSharingReport) {
    if report.model != Some(CacheModel::Best) {
        return;
    }

    println!("\nBest-Case Cache Sharing:");
    println!(
        "  Backing memory traffic: {} ({:.2}) -> {} ({:.2}), avoided {} ({:.2}).",
        report.original_memory_bytes,
        format_bytes(report.original_memory_bytes),
        report.adjusted_memory_bytes,
        format_bytes(report.adjusted_memory_bytes),
        report.memory_bytes_avoided,
        format_bytes(report.memory_bytes_avoided)
    );
    if report.shared_lines.is_empty() {
        println!("  No cross-PE shared tensor reads found on shared cache paths.");
        return;
    }

    println!("  Top shared cache lines:");
    for line in report.shared_lines.iter().take(10) {
        println!(
            "    {} line 0x{:x} from {}: pes=[{}] nodes=[{}] read={} ({:.2}) backing={} ({:.2}) avoided={} ({:.2})",
            line.cache_name,
            line.line_addr,
            line.memory_name,
            line.pes.join(", "),
            line.node_ids.join(", "),
            line.read_bytes,
            format_bytes(line.read_bytes),
            line.backing_memory_bytes,
            format_bytes(line.backing_memory_bytes),
            line.memory_bytes_avoided,
            format_bytes(line.memory_bytes_avoided)
        );
    }
}
...
pub fn print_roofline_report(
    ...
    print_cache_sharing_report(&report.cache_sharing);
