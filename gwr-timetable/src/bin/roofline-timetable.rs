...
impl From<CacheModelArg> for CacheModel {
    fn from(value: CacheModelArg) -> Self {
        match value {
            CacheModelArg::Worst => CacheModel::Worst,
            CacheModelArg::Best => CacheModel::Best,
        }
    }
}
...
struct Args {
    ...
    #[arg(long = "no-scheduled-runtime", default_value_t = false)]
    ...
    /// Cache sharing bound to use when computing memory traffic.
    #[arg(long, value_enum, default_value_t = CacheModelArg::Worst)]
    cache_model: CacheModelArg,
    ...
    /// Write per-PE roofline metrics for gwr-visualisation to this JSON file.
    #[arg(long)]
    metric_overlay: Option<PathBuf>,
}
...
fn main() -> Result<()> {
    ...
    let report = analyzer.analyze_with_options(
        ...
        RooflineAnalysisOptions {
            ...
            cache_model: args.cache_model.into(),
        },
    )?;
    if args.timings {
        eprintln!(
    ...
    if let Some(path) = &args.metric_overlay {
        MetricOverlay::from_roofline_report(&clock, &report).write_json(path)?;
        println!("Wrote metric overlay to '{}'.", path.display());
    }
    if args.timings {
