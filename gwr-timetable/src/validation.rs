...
pub(crate) fn wire_nodes(
    ...
    for edge in edges {
        ...
        update_edge_indices(from_node_idx, to_edge_idx, &mut nodes[to_node_idx].inputs).map_err(
            |error| {
                SimError(format!(
...
fn validate_no_read_write_overlap(nodes: &[Node], node: &Node) -> SimResult {
    ...
    for read in &reads {
        for write in &writes {
            if read.range.start < write.range.end && write.range.start < read.range.end {
                return sim_error!(
