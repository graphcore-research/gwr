...
#[test]
fn platform_config_validate_rejects_duplicate_section_names() {
    let cases = [
        (
            "
memory_maps:
  ...
  - name: duplicate
    devices: []
",
            "Duplicate memory map name duplicate",
        ),
        (
            "
memory_maps: []
...
fabrics:
  ...
  - name: duplicate
    ...
    rows: 1
",
            "Duplicate fabric name duplicate",
        ),
    ];

    for (yaml, expected_error) in cases {
        let platform: PlatformConfig = serde_yaml::from_str(yaml).unwrap();

        let error = platform.validate().unwrap_err();
...
#[test]
fn platform_config_validate_rejects_pe_as_memory_map_device() {
    let platform: PlatformConfig = serde_yaml::from_str(
        "
memory_maps:
  - name: mm0
...
",
    ...
    assert!(format!("{error}").contains("Unknown memory 'pe0' in memory map 'mm0'"));
}

#[test]
fn platform_config_validate_rejects_memory_as_pe_endpoint() {
    let platform: PlatformConfig = serde_yaml::from_str(
        "
memory_maps: []
...
",
    ...
    assert!(format!("{error}").contains("No PE 'mem0'"));
}
...
#[test]
fn memory_range_rejects_last_address_overflow() {
    let platform: PlatformConfig = serde_yaml::from_str(
        "
memory_maps: []
memories:
...
#[test]
fn adjacent_memories_can_end_at_top_of_physical_address_space() {
    let platform: PlatformConfig = serde_yaml::from_str(
        "
memory_maps: []
memories:
...
#[test]
fn platform_config_rejects_referenced_zero_capacity_memory() {
    let platform: PlatformConfig = serde_yaml::from_str(
        "
memory_maps:
  - name: mm0
...
",
    ...
    let err = platform.validate().unwrap_err();
    assert!(format!("{err}").contains("Memory 'hbm0' has zero capacity"));
}

#[test]
fn platform_config_rejects_unreferenced_zero_capacity_memory() {
    let platform: PlatformConfig = serde_yaml::from_str(
        "
memory_maps: []
memories:
...
",
    ...
    let err = platform.validate().unwrap_err();
    assert!(format!("{err}").contains("Memory 'unused' has zero capacity"));
}
...
",
    ...
    assert!(message.contains("'hbm0' (0x0..=0x3ff)"));
    assert!(message.contains("'hbm1' (0x200..=0x5ff)"));
}
...
#[test]
fn platform_config_validate_rejects_unconnected_single_port_fabric() {
    let platform: PlatformConfig = serde_yaml::from_str(
        "
memory_maps: []
fabrics:
...
",
    ...
    let err = platform.validate().unwrap_err();
    assert!(format!("{err}").contains("Fabric 'fabric0' has 1 total ports"));
}

#[test]
fn platform_config_validate_accepts_unconnected_two_port_fabric() {
    let platform: PlatformConfig = serde_yaml::from_str(
        "
memory_maps: []
fabrics:
...
#[test]
fn platform_config_validate_rejects_fabric_port_count_overflow() {
    let yaml = format!(
        "
memory_maps: []
fabrics:
...
",
    ...
    let err = platform.validate().unwrap_err();
    assert!(format!("{err}").contains("Fabric 'fabric0' port count overflows"));
}
