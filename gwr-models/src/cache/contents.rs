...
use super::line_state::{
    LineState, LineStateGrantExclusiveCleanTransition, LineStateGrantExclusiveModifiedTransition,
    LineStateInvalidateLineTransition, LineStateTransition,
};
    ...
    T: SimObject + CoherentAccess,
{
    ...
    pub(super) fn evict_modified(&mut self, addr: u64) -> bool {
        ...
        if entry.apply::<LineStateInvalidateLineTransition>() {
            entry.tag = 0;
            return true;
        }
    ...
    pub(super) fn grant_exclusive(&mut self, addr: u64, modified: bool) -> bool {
        if modified {
            self.transition::<LineStateGrantExclusiveModifiedTransition>(addr)
        } else {
            self.transition::<LineStateGrantExclusiveCleanTransition>(addr)
        }
    }
    ...
    pub(super) fn invalidate(&mut self, addr: u64) {
        ...
        for i in 0..self.config.num_ways {
            if self.sets[set_index][i].tag == tag {
                if self.sets[set_index][i].apply::<LineStateInvalidateLineTransition>() {
...
mod tests {
    ...
    use super::super::line_state::{
        LineState, LineStateAllocateExclusiveTransition, LineStateAllocateSharedTransition,
        LineStateGrantSharedTransition,
    };
    ...
    fn basic_ways() {
        ...
        for addr in addrs.iter().take(num_ways) {
            ...
            assert!(
                state
                    .allocate::<LineStateAllocateSharedTransition>(*addr)
                    .is_allocated()
            );
        ...
        assert!(
            state
                .allocate::<LineStateAllocateSharedTransition>(addrs[num_ways])
                .is_blocked()
        );
        for addr in addrs.iter().take(num_ways) {
            assert_eq!(state.state_for(*addr), Some(LineState::AllocatedShared));
    ...
    fn cache_contents_reports_outstanding_work_from_allocations_and_pending_line_waiters() {
        ...
        state.transition::<LineStateGrantSharedTransition>(addr);
        assert!(!state.has_allocated_lines());
        assert!(!state.has_outstanding_work());
    ...
    fn cache_contents_updates_existing_entries_and_reuses_evictable_ways() {
        ...
        assert!(
            state
                .allocate::<LineStateAllocateSharedTransition>(first)
                .is_allocated()
        );
        assert_eq!(
        ...
        assert!(
            state
                .allocate::<LineStateAllocateExclusiveTransition>(second)
                .is_allocated()
        );
        assert!(
            state
                .allocate::<LineStateAllocateSharedTransition>(third)
                .is_blocked()
        );

        assert!(state.transition::<LineStateGrantSharedTransition>(first));
    ...
    fn modified_victim_must_be_explicitly_evicted_before_reallocation() {
        ...
        assert!(
            state
                .allocate::<LineStateAllocateExclusiveTransition>(first)
                .is_allocated()
        );
        assert!(state.grant_exclusive(first, true));
        assert_eq!(state.state_for(first), Some(LineState::Modified));

        let allocate_result = state.allocate::<LineStateAllocateSharedTransition>(second);
        ...
        assert!(
            state
                .allocate::<LineStateAllocateSharedTransition>(second)
                .is_allocated()
        );
