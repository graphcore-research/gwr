...
use line_state::{
    ...
    LineStateGrantSharedTransition, LineStateTransition,
};
    ...
    T: SimObject + CoherentAccess,
{
    ...
    match line_state {
        ...
        None => {
            ...
            let allocate_result = allocate_line::<_, LineStateAllocateSharedTransition>(
                state, mem_ack, &request, addr,
            )
            .await?;
    ...
    T: SimObject + CoherentAccess,
{
    ...
    match line_state {
        ...
        Some(LineState::Shared) | None => {
            ...
            let allocate_result = allocate_line::<_, LineStateAllocateExclusiveTransition>(
                state, mem_ack, &request, addr,
            )
            .await?;
            match allocate_result {
    ...
    if !completed_noallocate {
        if access.coherence_op() == Some(CoherenceOp::GrantExclusive) {
            ...
            state
                .contents
                .borrow_mut()
                .transition::<LineStateGrantSharedTransition>(access.dst_addr());
        }
    }
    process_pending_line_waiters_for_line(state, dev_to_mem, rsp_arb_0, &access).await
}
